...
def DES(text, key):
    ...
    return finalTextOfBit


def chooseChange(bit, key, text, choose):
    ...
    text = bitText
    key = char2unicode_ascii(key, len(key))
    key = byte2bit(key, len(key))
    result1 = DES(text, key)
    if choose == 1:
        test = text.copy()
    else:
        test = key.copy()
    bitchange = set()
    while len(bitchange) < bit:
        ci = random.randint(0, 63)
        if ci in bitchange:
            continue
        else:
            test[ci] ^= 1
            bitchange.add(ci)
    if choose == 1:
        result2 = DES(test, key)
    else:
        result2 = DES(text, test)
    cnt = 0
    for i in range(0, 64):
...
class TestDesSource(unittest.TestCase):

    def testRun(self):
        # print(chooseChange(64, "aaaaaaaa", "bbbbbbbb", 1))
        print(chooseChange(1, "aaaaaaaa", "bbbbbbbb", 2))
